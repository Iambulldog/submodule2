    ...
    <div>
        ...
        <main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-10">
                <h1 class="text-4xl font-bold tracking-tight text-gray-900">Shopping</h1>
            </div>
            ...
            <section aria-labelledby="products-heading" class="pb-24 pt-6">
                ...
                <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                        ...
                        <div class="border-b border-gray-200 py-6">
                            ...
                            <div class="pt-6" id="filter-section-1">
                                <div class="space-y-4">
                                    ...
                                    @foreach($get_brand as $keyBrand => $valueBrand)
                                    <div class="flex items-center">
                                        <input name="brand" wire:click="setName('{{ $valueBrand['name'] }}')"
                                            type="radio"
                                            class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-600">
                                        <label for="filter-category-0" class="ml-3 text-sm text-gray-600">{{$valueBrand['name']}}</label>
                                    </div>
                                    @endforeach
                    ...
                    <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:col-span-3 lg:gap-x-8">
                        @foreach ($data_shopping as $key => $value)
                            <a href="#" class="group text-sm" wire:click="$emit('openModalConfrimShopping' , {{json_encode($value)}})">
                                <div
                                    class="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-100 group-hover:opacity-75">
                                    <img src="{{ $value['product_img'] ? $value['product_img'][0] : '' }}"
                                        alt="White fabric pouch with white zipper, black zipper pull, and black elastic loop."
                                        class="h-full w-full object-cover object-center">
                                </div>
                                <h3 class="mt-4 font-medium text-gray-900">{{ $value['name'] }}</h3>
                                    @isset($value['product_attr']['color'])
                                        <p class="italic text-gray-500">{{ $value['product_attr']['color'] }}</p>
                                    @endisset
                                <p class="mt-2 font-medium text-gray-900">{{ $value['product_attr']['price'] }}</p>
                            </a>
                        @endforeach
                        <!-- More products... -->
                    </div>
...
<livewire:member.shopping.modal-confrim-shopping />

<script>
    document.addEventListener('livewire:load', function() {
        Livewire.on('SuccessUserShopping', function() {
            Swal.fire('สำเร็จ', 'ทำรายการซื้อสินค้าสำเร็จ', 'success')
        });
        Livewire.on('ErrorUserShopping', function() {
            Swal.fire('ไม่สำเร็จ', 'ไม่สามารถทำรายการซื้อสินค้าได้', 'error')
        });
    });
</script>
